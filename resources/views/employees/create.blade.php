...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="flex justify-between items-center mb-8">
        <h2 class="text-3xl font-bold text-pink-600">Crear Nuevo Empleado</h2>
    ...
    <div class="bg-white shadow-xl rounded-3xl p-8 border border-pink-100">
        <form action="{{ route('employees.store') }}" method="POST" class="space-y-7">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" 
                           ...
                           class="w-full border border-pink-200 rounded-2xl px-4 py-3 bg-pink-50/40 focus:ring-2 focus:ring-pink-500 text-pink-700">
                </div>
            ...
            <div>
                ...
                <select name="specialty"
                    ...
                    @foreach($specialties as $spec)
                        <option value="{{ $spec }}" @selected(old('specialty') == $spec)>{{ $spec }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label class="block mb-2 font-semibold text-pink-700">Hora de Inicio</label>
                    <input type="time" 
                           name="work_start" 
                ...
                <div>
                    <label class="block mb-2 font-semibold text-pink-700">Hora de Fin</label>
                    <input type="time" 
                           name="work_end" 
                           value="{{ old('work_end') }}"
                           class="w-full border border-pink-200 rounded-2xl px-4 py-3 bg-pink-50/40 text-pink-700 focus:ring-2 focus:ring-pink-500">
                    ...
                    @foreach($weekDays as $key=>$label)
                        <label class="flex items-center gap-2 text-pink-700">
                            <input type="checkbox" 
                                   name="days_off[]" 
                                   value="{{ $key }}"
                                   @checked(is_array(old('days_off')) && in_array($key, old('days_off')))
