...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="flex justify-between items-center mb-8">
        <h2 class="text-3xl font-bold text-pink-600">✨ Editar Rol</h2>
    ...
    <div class="bg-white shadow-xl rounded-3xl p-8 border border-pink-100">
        <form action="{{ route('roles.update', $role->id) }}" method="POST" class="space-y-7">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select
                    ...
                    required
                >
                    <option value="1" {{ old('status', $role->status) == 1 ? 'selected' : '' }}>Activo</option>
                    <option value="0" {{ old('status', $role->status) == 0 ? 'selected' : '' }}>Inactivo</option>
                </select>
            </div>
            ...
            <div class="flex justify-end gap-3 pt-4">
                ...
                <button
                    type="submit"
                    class="bg-gradient-to-r from-pink-500 to-pink-700 hover:to-pink-800 text-white px-6 py-2 rounded-full shadow-md transition">
                    Actualizar Rol
                </button>
